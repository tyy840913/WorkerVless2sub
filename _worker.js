...
for (const { host: hostName, ips } of hostIPMap) {
  for (const addrItem of ips) {
    ...
    if (!match) {
      if (addrItem.includes(':') && addrItem.includes('#')) {
        const [left, remark] = addrItem.split('#');
        addressid = remark;
        [address, port] = left.split(':');
      } else if (addrItem.includes(':')) {
        [address, port] = addrItem.split(':');
      } else if (addrItem.includes('#')) {
        [address, addressid] = addrItem.split('#');
      }
    } else {
      address = match[1];
      port = match[2] || port;
    ...
    if (!isValidIPv4(address) && port === '443') {
      for (const p of httpsPorts) if (addrItem.includes(p)) { port = p; break; }
    }

    /* ****** 生成路径（proxyip 逻辑保持与原脚本一致） ****** */
    let finalPath = path;
    let fakeHost = hostName;
    ...
    /* ****** 按协议类型生成链接 ****** */
    const remark = `${addressid} | ${hostName}${nodeRemark}`;
    if (协议类型 === 'VMess') {
...
if ((noTLS === 'true' && 协议类型 === atob('VkxFU1M=')) || 协议类型 === 'VMess') {
  ...
  for (const hostName of hosts) {
    for (let i = 0; i < 5; i++) {
      ...
      nlsIpCursor++;
      // 解析与生成逻辑与上面 TLS 部分完全一致，仅 tls 字段为空
      let [address, port = '80'] = addrItem.includes(':') ? addrItem.split(':') : [addrItem, '80'];
      let addressid = addrItem.split('#')[1] || address;
      const remark = `${addressid} | ${hostName}${EndPS}`;
      if (协议类型 === 'VMess') {
        notlsNodes.push(`vmess://${utf8ToBase64(JSON.stringify({
          v: '2', ps: remark, add: address, port, id: uuid, aid: 额外ID, scy: 加密方式,
          net: 'ws', type, host: hostName, path, tls: '', sni: '', alpn: encodeURIComponent(alpn), fp: ''
        }))}`);
      } else {
        notlsNodes.push(`${atob(atob('ZG14bGMzTTZMeTg9'))}${uuid}@${address}:${port}?` +
          `security=&type=${type}&host=${hostName}&path=${encodeURIComponent(path)}&encryption=none#${encodeURIComponent(remark)}`);
      }
    }
  }
