...
'Accept': 'text/html,application/xhtml+xml,application/xml;'
                ,
'User-Agent':
                FileName + atob('IChodHRwczovL2dpdGh1Yi5jb20vY21saXUvV29ya2VyVmxlc3Myc3ViKQ==')
            },
signal:
            controller.signal // 将AbortController的信号量添加到fetch请求中，以便于需要时可以取消请求
...
const formattedAddress = `$ {ipAddress}:
                                         $ {port}#$ {dataCenter}`;
...
const 正确内容 = `skip-cert-verify=true, ws=true, ws-path=$ {path}, ws-headers=Host:"${host}"
                                                   , tfo=false, udp-relay=false`;
...
'Accept': 'text/html,application/xhtml+xml,application/xml;'
                    ,
'User-Agent': 'v2rayN/'
                    + FileName + ' (https://github.com/cmliu/WorkerVless2sub)'
                    },
signal:
                controller.signal // 将AbortController的信号量添加到fetch请求中
            }).then(response => response.ok ? response.text() : Promise.reject())));
...
const 为烈士Link = `$ {atob(atob('ZG14bGMzTTZMeTg9')) + uuid}@$ {address}:
                                              $ {port}?security=&type=$ {type}&host=$ {host}&path=$ {encodeURIComponent(path)}&encryption=none#$ {encodeURIComponent(addressid + EndPS)}`;
                        return 为烈士Link;
                    }
            ...
            /* ****************************************************
            * 1. 准备：拿到所有 host 与所有优选 IP
            * ****************************************************/
            const hosts = env.HOST ? await 整理(env.HOST) : [];
            if (hosts.length === 0) throw new Error('HOST 变量未配置或为空');

// 取全部优选 IP（含 API + CSV），并打乱
            const newAddressesapi = await 整理优选列表(addressesapi);
            const newAddressescsv = await 整理测速结果('TRUE');
            const allIPs = [...new Set([...newAddressesapi, ...newAddressescsv])]
                           .filter(Boolean)
                           .sort(() => 0.5 - Math.random());

            if (allIPs.length === 0) throw new Error('ADDAPI / ADDCSV 未返回任何优选 IP');

            /* ****************************************************
             * 2. 每个 host 分配 5 个 IP（不足则循环使用）
             * ****************************************************/
            const hostIPMap = [];
            let ipCursor = 0;
            for (const host of hosts) {
                const assignedIPs = [];
                for (let i = 0; i < 5; i++) {
                    assignedIPs.push(allIPs[ipCursor % allIPs.length]);
                    ipCursor++;
                }
                hostIPMap.push({ host, ips: assignedIPs });
            }
            ...
            for (const { host, ips } of hostIPMap) {
                for (const addrItem of ips) {
                    let address = addrItem;
                    let port = '443';
                    let addressid = addrItem;

                    // 统一解析 address:port#remarks
                    const match = addressid.match(regex);
                    if (!match) {
                        if (addressid.includes(':') && addressid.includes('#')) {
                            const [left, remark] = addressid.split('#');
                            addressid = remark;
                            [address, port] = left.split(':');
                        } else if (addressid.includes(':')) {
                            [address, port] = addressid.split(':');
                        } else if (addressid.includes('#')) {
                            [address, addressid] = addressid.split('#');
                        }
                    } else {
                        address = match[1];
                        port = match[2] || port;
                    ...
                    if (!isValidIPv4(address) && port === '443') {
                        // 非 IPv4 且未指定端口时尝试识别 httpsPorts
                        for (const p of httpsPorts) if (addrItem.includes(p)) {
                                port = p;
                                break;
                            }
                    }

                    /* ****** 生成路径（proxyip 逻辑保持与原脚本一致） ****** */
                    let finalPath = path;
                    let fakeHost = host;
                    let nodeRemark = EndPS;

                    if (隧道版本作者.trim() === atob('Y21saXU=') && 获取代理IP.trim() === 'true') {
                        const lowerId = addressid.toLowerCase();
                        let foundProxyIP = null;
                        for (const item of 匹配PROXYIP) {
                            if (item.includes('#') && lowerId.includes(item.split('#')[1]?.toLowerCase())) {
                                foundProxyIP = item.split('#')[0];
                                break;
                            }
                            if (item.includes(':') && lowerId.includes(item.split(':')[1]?.toLowerCase())) {
                        ...
                        if (matchingProxyIP)       finalPath = atob('L3Byb3h5aXA9') + matchingProxyIP;
                        else if (foundProxyIP)     finalPath = atob('L3Byb3h5aXA9') + foundProxyIP;
                        else if (proxyIPs.length)  finalPath = atob('L3Byb3h5aXA9') + proxyIPs[Math.floor(Math.random() * proxyIPs.length)];
                    }

                    /* ****** 临时中转域名逻辑保持与原脚本一致 ****** */
                    if (临时中转域名.length && fakeHost.includes('.workers.dev')) {
                        finalPath = `/$ {fakeHost}$ {finalPath}`;
                        fakeHost = 临时中转域名[Math.floor(Math.random() * 临时中转域名.length)];
                        nodeRemark += atob('IOW3suWQr+eUqOS4tOaXtuWfn+WQjeS4rei9rOacjeWKoe+8jOivt+WwveW/q+e7keWumuiHquWumuS5ieWfn++8gQ==');
                    }

                    /* ****** 按协议类型生成链接 ****** */
                    const remark = `$ {addressid} | $ {host}$ {nodeRemark}`;
                    if (协议类型 === 'VMess') {
                        allNodes.push(`vmess://${utf8ToBase64(JSON.stringify({
                                      v: '2', ps: remark, add: address, port, id: uuid, aid: 额外ID, scy: 加密方式,
                                      net: 'ws', type, host: fakeHost, path: finalPath, tls: 'tls', sni: fakeHost,
                                      alpn: encodeURIComponent(alpn), fp: '', allowInsecure: scv === 'true' ? '1' : '0',
                                      fragment: '1,40-60,30-50,tlshello'
                    }))
                }`);
            } else if (协议类型 === atob('VHJvamFu')) {
                allNodes.push(`$ {atob(atob('ZEhKdmFtRnVPaTh2'))}$ {uuid}@$ {address}:$ {port}?` +
                              `security=tls&sni=$ {fakeHost}&alpn=$ {encodeURIComponent(alpn)}&fp=random&` +
                                                `type=$ {type}&host=$ {fakeHost}&path=$ {encodeURIComponent(finalPath)}$ {scv === 'true' ? '&allowInsecure=1' : ''}&` +
                                                        `fragment=$ {encodeURIComponent('1,40-60,30-50,tlshello')}#$ {encodeURIComponent(remark)}`);
            } else { // VLESS
                allNodes.push(`$ {atob(atob('ZG14bGMzTTZMeTg9'))}$ {uuid}@$ {address}:$ {port}?` +
                              `security=tls&sni=$ {fakeHost}&alpn=$ {encodeURIComponent(alpn)}&fp=random&` +
                                                `type=$ {type}&host=$ {fakeHost}&path=$ {encodeURIComponent(finalPath)}$ {xhttp}$ {scv === 'true' ? '&allowInsecure=1' : ''}&` +
                                                        `fragment=$ {encodeURIComponent('1,40-60,30-50,tlshello')}&encryption=none#$ {encodeURIComponent(remark)}`);
            }
        }
    }

    /* ****************************************************
     * 4. 输出合并（link / notls 逻辑保持与原脚本一致）
     * ****************************************************/
    let combinedContent = allNodes.join('\n');
...
// 追加 notls 节点
    ...
    if ((noTLS === 'true' && 协议类型 === atob('VkxFU1M=')) || 协议类型 === 'VMess') {
        const newAddressesnotlsapi = await 整理优选列表(addressesnotlsapi);
        const newAddressesnotlscsv = await 整理测速结果('FALSE');
        const notlsIPs = [...new Set([...addressesnotls, ...newAddressesnotlsapi, ...newAddressesnotlscsv])]
                         .filter(Boolean)
                         .sort(() => 0.5 - Math.random());
        ...
        for (const host of hosts) {
            for (let i = 0; i < 5; i++) {
                const addrItem = notlsIPs[nlsIpCursor % notlsIPs.length];
                nlsIpCursor++;
                // 下面解析与生成逻辑与上面 TLS 部分完全一致，仅 tls 字段为空
                let [address, port='80'] = addrItem.includes(':') ? addrItem.split(':') : [addrItem,'80'];
                let addressid = addrItem.split('#')[1] || address;
                // ...（省略，完全可复制上面解析段）
                const remark = `$ {addressid} | $ {host}$ {EndPS}`;
                if (协议类型 === 'VMess') {
                    notlsNodes.push(`vmess://${utf8ToBase64(JSON.stringify({
                                    v:'2',ps:remark,add:address,port,id:uuid,aid:额外ID,scy:加密方式,
                                    net:'ws',type,host:host,path,tls:'',sni:'',alpn:encodeURIComponent(alpn),fp:''
                }))
            }`);
        } else {
            notlsNodes.push(`$ {atob(atob('ZG14bGMzTTZMeTg9'))}$ {uuid}@$ {address}:$ {port}?` +
                            `security=&type=$ {type}&host=$ {host}&path=$ {encodeURIComponent(path)}&encryption=none#$ {encodeURIComponent(remark)}`);
        }
    }
}
notlsContent = notlsNodes.join('\n');
combinedContent += '\n' + notlsContent;
}
...
try {
    ...
    if (!userAgent.includes('mozilla')) responseHeaders["Content-Disposition"] = `attachment;
